...
namespace Cilador.Clone
{
    ...
    internal class PropertyCloner : ClonerBase<PropertyDefinition>
    {
        ...
        protected override void DoClone()
        {
            ...
            this.Target.PropertyType = this.CloningContext.RootImport(this.Source.PropertyType);

            // seting the getter and setter methods also sets the parameters for the property
            if (this.Source.GetMethod != null)
            {
                var targetGetMethod = this.CloningContext.RootImport(this.Source.GetMethod) as MethodDefinition;
            ...
            {
                var targetSetMethod = this.CloningContext.RootImport(this.Source.SetMethod) as MethodDefinition;
