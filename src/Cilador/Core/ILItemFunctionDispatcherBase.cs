...
{
    /// <summary>
    /// Base for types that define behavior to be run against IL items where
    /// the behavior may vary based on the item type.
    /// </summary>
    /// <typeparam name="TReturn">Type of the return value for the behavior.</typeparam>
    [ContractClass(typeof(ILItemFunctionDispatcherContract<>))]
    internal abstract class ILItemFunctionDispatcherBase<TReturn>
    {
        /// <summary>
        ...
        /// <summary>
        /// Invokes the behavior for items that are not IL items.
        /// </summary>
        /// <param name="item">Item on which to invoke the behavior.</param>
        /// <returns>Result of the invocation.</returns>
