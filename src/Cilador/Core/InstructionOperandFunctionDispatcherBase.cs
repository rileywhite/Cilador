...
namespace Cilador.Core
{
    ...
    /// with a breakout for important IL types
    /// </summary>
    ...
    internal abstract class InstructionOperandFunctionDispatcherBase<TReturn, TState>
    {
        ...
        public TReturn InvokeFor(Instruction instruction, TState state)
        {
            ...
            if (instruction.Operand == null) { return this.GetReturnValueForNull(instruction.OpCode, state); }
            return InvokeForOperand((dynamic)instruction.Operand, instruction.OpCode, state);
        }
